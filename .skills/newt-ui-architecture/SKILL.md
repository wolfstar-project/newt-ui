...
- `packages/newt-ui`: the `newt-ui` CLI (React) **and** `registry/html/`, the
  ...
  value.
- `packages/cli`: the `newt-ui-vue` CLI. Same command surface as `newt-ui`,
...
- `packages/module`: `@newt-devs/nuxt`, auto-imports a consumer's
