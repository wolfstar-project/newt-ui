...
2. **Framework marks are drawn, not fetched.** Simple monochrome paths in `apps/docs/src/components/site/marks/*.astro` (Next, Vite, Nuxt, Astro, TanStack, React Router, Laravel, React, Vue, a code-brackets glyph for HTML, a wrench for Manual). `currentColor`, so they follow the theme. React and Vue move out of `FrameworkSwitcher.tsx` into the same folder.
