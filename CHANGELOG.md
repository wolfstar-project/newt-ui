...
> **Where the user-facing changelog lives now.** Anything that changes what a
> reader installs — a component, a token, a CLI command — is written up at
> [newtui.dev/docs/changelog](https://newtui.dev/docs/changelog), which renders
> live examples alongside the entry. This file keeps the release history of the
> published packages; the per-package files under `packages/*/CHANGELOG.md` are
