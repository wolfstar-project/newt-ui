...
export async function init(options: InitOptions): Promise<void> {
  ...
  outro(
    `Success! Project initialization completed. You may now add components: ${highlighter.info("npx @newtui/vue add button")}`
  )
}
...
function printTailwindInstructions(
  ...
  if (major === 4) {
    ...
    if (!theme) {
      logger.warn(
        `The registry was unreachable, so only the raw ${highlighter.info("--newt-*")} variables were written. Re-run ${highlighter.info("@newtui/vue init")} when you are online to add the @theme block.`
      )
    }
    return
  }
  ...
  if (!extend) {
    logger.warn(
      `The registry was unreachable, so the v3 preset could not be printed. Re-run ${highlighter.info("@newtui/vue init")} when you are online.`
    )
    return
  }
...
function printNuxtInstructions(css: string): void {
  ...
  logger.log(
    highlighter.dim(
      "  Tip: @newtui/nuxt auto-imports every component in your ui directory — see https://wolfstar-project.github.io/newt-ui"
    )
  )
}
