...
import { getRegistryItem, getRegistryUrl } from "../registry/api.js"
...
function printNuxtInstructions(css: string): void {
  const cssAlias = toNuxtCssAlias(css)
  logger.log(highlighter.bold("Nuxt setup"))
  logger.log(
    `Make sure ${highlighter.info("@tailwindcss/vite")} is configured and your CSS entry is loaded in ${highlighter.info("nuxt.config.ts")}:`
  )
  logger.log(
    highlighter.dim(`  import tailwindcss from "@tailwindcss/vite"
...
export function toNuxtCssAlias(css: string): string {
  if (css.startsWith("~/")) return css
  return `~/${css.replace(/^\.\//, "").replace(/^(?:app|src)\//, "")}`
}
