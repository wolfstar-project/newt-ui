...
export async function diff(options: DiffOptions): Promise<void> {
  ...
  if (!config) {
    throw new Error(
      `Configuration is missing. Please run ${highlighter.info("npx @wolfstar/newt-ui-vue init")} first.`
    )
  }
  if (!options.component) {
    throw new Error(
      "Please specify a component name, e.g. `newt-ui-vue diff button`."
    )
  }
  ...
  outro(
    highlighter.dim(
      `Run ${highlighter.info(`npx @wolfstar/newt-ui-vue add ${item.name} --overwrite`)} to update.`
    )
  )
}
