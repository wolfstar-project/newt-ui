 ...
 * Uses tsconfig `paths` when available, otherwise falls back to `src/` or the project root.
 */
async function resolveImport(alias: string, cwd: string): Promise<string> {
...
export async function resolveConfigPaths(
  ...
  const utils = await resolveImport(config.aliases.utils, cwd)
  const components = await resolveImport(config.aliases.components, cwd)
  const ui = config.aliases.ui
    ? await resolveImport(config.aliases.ui, cwd)
    : path.resolve(components, "ui")
  const lib = config.aliases.lib
    ? await resolveImport(config.aliases.lib, cwd)
    : path.dirname(utils)
  const hooks = config.aliases.hooks
    ? await resolveImport(config.aliases.hooks, cwd)
