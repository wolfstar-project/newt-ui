...
export async function add(options: AddOptions): Promise<void> {
  ...
  if (!config) {
    throw new Error(
      `Configuration is missing. Please run ${highlighter.info("npx @newtui/react init")} to create a components.json file.`
    )
  }
 ...
 *
 * Most components need nothing here — they are styled entirely by the tokens
 * `init` wrote. This exists for items that add their own custom properties or
 * `@layer` rules, and it renders them for whichever Tailwind major the project
 * is on. Blocks already present are not written twice.
 */
