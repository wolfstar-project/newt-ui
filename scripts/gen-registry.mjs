...
const reactUi = metas.map((m) => ({
  ...
  ...(m.registryDependencies?.length
    ? { registryDependencies: m.registryDependencies }
    : {}),
  files: [{ path: `ui/${m.name}.tsx`, type: "registry:ui" }],
}))
...
const vueUi = metas.map((m) => ({
  ...
  ...(m.registryDependencies?.length
    ? { registryDependencies: m.registryDependencies }
    : {}),
  files: (m.vueFiles ?? [`${pascal(m.name)}.vue`, "index.ts"]).map((f) => ({
    path: `ui/${m.name}/${f}`,
    type: "registry:ui",
  })),
}))
...
const registryJson = {
  ...
  name: "newt-ui",
  homepage: "https://newt-devs.github.io/newt-ui",
  items: reactUi
    .map((i) =>
      Object.assign({}, i, {
        files: i.files.map((f) => ({
