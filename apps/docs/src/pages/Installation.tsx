...
export function Installation() {
  ...
  return (
    <>
      <PageHead
        eyebrow={[SITE.name, `v${SITE.version}`, "getting started"]}
        overline="setup —"
        title="Installation"
        lead="Two commands. The first reads your project and writes the tokens, the second writes a component and everything it imports."
      />
      ...
      <Content>
        <Section
          ...
          description="The versions the CLI expects, and the packages it installs on your behalf."
        >
          <List>
            <li>
              ...
              <FrameworkText
                react={
                  <span>
                    ...
                    <InlineCode>@theme</InlineCode> block instead and asks you
                    for nothing.
                  </span>
                }
                vue={
                  <span>
                    ...
                    <InlineCode>init</InlineCode> writes. It detects the major
                    of whatever project it runs in, and on v3 it prints a preset
                    for your <InlineCode>tailwind.config.ts</InlineCode>{" "}
                    instead.
                  </span>
                }
              />
          ...
          <Note>
            ...
            two stylesheets and a class name, with no build step, no framework
            and no package manager — it is the last section on this page.
          </Note>
        </Section>
        ...
        <Section
          id="init"
          title="Run init once"
          description="What init reads, what it writes, and the questions it asks before it does either."
        >
          <CodeBlock shell lang="bash" code={INIT} />
          <P>
            It asks a handful of questions and then writes three things.{" "}
            <InlineCode>components.json</InlineCode> at the root of your
            project, which is the same file shadcn reads, so a project using
            both CLIs keeps one config and not two. A{" "}
            <InlineCode>cn</InlineCode> helper at the path your utils alias
            names — a re-export of the <InlineCode>cn</InlineCode> package on
            Tailwind v4, and <InlineCode>clsx</InlineCode> passed through{" "}
            ...
            <FrameworkText
              react={
                <span>
                  ...
                  path it detected, and two switches:{" "}
                  <InlineCode>tsx</InlineCode>, and <InlineCode>rsc</InlineCode>{" "}
                  for React Server Components. Both change what{" "}
                  <InlineCode>add</InlineCode> writes later. The default aliases
                  are <InlineCode>@/components</InlineCode>,{" "}
              ...
              vue={
                <span>
                  The config records your aliases, the stylesheet and the config
                  path it detected, whether you are on TypeScript, and whether
                  the project is Nuxt or Vite — it guesses that from a{" "}
        ...
        <Section
          id="add"
          title="Then add what you need"
          description="One command per component. Registry dependencies come along with it."
        >
          <CodeBlock shell lang="bash" code={ADD} />
          <P>
            Registry dependencies are resolved before anything is written, so
            you name what you want and get what it is built out of. The modal
            above names the button, which means{" "}
            <InlineCode>add modal</InlineCode> on its own writes both. The npm
            packages the components import are collected across the whole tree
            and installed in one pass; pass{" "}
            <InlineCode>--skip-install</InlineCode> and the files still land and
            your package manager is left alone.
          </P>
          <P>
            A file already on disk is kept, not replaced: you are asked, and
            with <InlineCode>--yes</InlineCode> it is skipped and reported.{" "}
        ...
        <Section
          ...
          description="The files the CLI writes, at the paths your aliases name."
        >
          <P>
            Every path below the blank line is relative to whatever your aliases
            resolve to, so a project with a <InlineCode>src</InlineCode>{" "}
            directory gets them under <InlineCode>src</InlineCode>. The two
            paths above it are the ones <InlineCode>init</InlineCode> detected
            and recorded.
          </P>
          ...
          <P>
            <FrameworkText
              react={
                <span>
                  One file per component, and the imports inside it are
                  rewritten on the way in:{" "}
                  <InlineCode>@/registry/default/ui/button</InlineCode> becomes
                  your ui alias, and <InlineCode>@/lib/utils</InlineCode>{" "}
                  becomes your utils alias. The variants are{" "}
              ...
              vue={
                <span>
                  ...
                  along with the{" "}
                  <InlineCode>class-variance-authority</InlineCode> variants.
                  The imports are rewritten on the way in, so{" "}
          ...
          <Note>
            Nothing here is a dependency on {SITE.name}. The files are yours
            from the moment they are written, and deleting{" "}
        ...
        <Section
          ...
          description="One set of custom properties every component reads. Override them here rather than in the components."
        >
          <P>
            Every colour, radius, shadow, font and duration in the library is a{" "}
            <InlineCode>--newt-*</InlineCode> custom property. The source of
            truth is one file in the repository,{" "}
            <InlineCode>packages/newtui/registry/html/tokens.css</InlineCode>,
          ...
          <P>
            ...
            , which sets the font family, the base text colour and font
            smoothing. Put it on the element that wraps your application. Then
            make sure the stylesheet <InlineCode>init</InlineCode> wrote into is
            actually loaded:
          </P>
          <FrameworkBlock
            lang="typescript"
          ...
          />
          <Note>
            Overriding a token is a one-line change in your own stylesheet, and
            it reaches every component at once. Hardcoding a hex that already
            exists as a token is the one thing worth avoiding: it is the only
            way to end up with a component that no longer follows the rest.
          </Note>
        </Section>
        ...
        {vue && (
          <Section
            id="nuxt"
            title="On Nuxt"
            description="The module registers the components and the token stylesheet for you."
          >
            <P>
              <InlineCode>{SITE.nuxtModule}</InlineCode> saves you the imports.
              It registers your <InlineCode>components/ui</InlineCode> directory
              with Nuxt so <InlineCode>Button.vue</InlineCode> is available as{" "}
              <InlineCode>&lt;Button /&gt;</InlineCode> anywhere, without the
              directory name in front of it, and it prepends the token
              stylesheet to your CSS so the variables are there before your own
              styles run. It needs Nuxt 4.
            </P>
            <CodeBlock code={NUXT_CONFIG} lang="typescript" />
            <P>
              The three options are all optional.{" "}
              <InlineCode>prefix</InlineCode> namespaces the auto-imported names
              if a bare <InlineCode>&lt;Button /&gt;</InlineCode> would collide
              with something you already have,{" "}
              <InlineCode>componentDir</InlineCode> points at your components
              directory relative to the Nuxt source directory, and{" "}
              <InlineCode>css: false</InlineCode> turns the injected tokens off
              when your own CSS entry already carries them.
            </P>
          </Section>
        )}
        ...
        {!vue && (
          <Section
            id="next"
            title="On Next and the App Router"
            description="Where the token stylesheet goes, and which components need the client boundary."
          >
            <P>
              <InlineCode>init</InlineCode> looks for{" "}
              <InlineCode>app/globals.css</InlineCode> first, so on a standard
              App Router project the tokens land in the stylesheet your layout
              already imports. The class goes on the body:
            </P>
            ...
            <P>
              ...
              answering no sets <InlineCode>rsc: false</InlineCode> in your
              config and <InlineCode>add</InlineCode> strips them as it writes.
            </P>
          </Section>
        )}
        ...
        <Section
          id="html"
          title="Without a bundler"
          description="The plain HTML and CSS build, loaded from a CDN or copied in whole."
        >
          <P>
          ...
          <P>
            ...
            <InlineCode>styles/newt-tokens.css</InlineCode>; the two paths in
            that config are where everything afterwards goes.{" "}
            <InlineCode>add</InlineCode> copies a <InlineCode>.css</InlineCode>{" "}
            and a <InlineCode>.html</InlineCode> file per component into{" "}
            <InlineCode>components/ui</InlineCode>, following the same
            dependency links as the registry. The HTML file is the markup to
            copy:
          </P>
        ...
        <Section
          id="options"
          title="Options"
          description="Every flag the CLI accepts, and the commands each one belongs to."
        >
          <CodeBlock code={FLAGS} />
          <P>
            <InlineCode>--registry</InlineCode> overrides where items are
            fetched from, and so does the{" "}
            <InlineCode>NEWT_REGISTRY_URL</InlineCode> environment variable. The
            default is{" "}
            <InlineCode>
        ...
        <P>
          ...
          <span>
            {" "}
            is the shortest way to see what one of these files looks like once
            it is in your project. The registry, both CLIs and the Nuxt module
            all live in{" "}
          </span>
          <a
            href={SITE.github}
